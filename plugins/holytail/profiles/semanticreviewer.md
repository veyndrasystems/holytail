...
`FULL`; `ultra` is not a Holytail signal; neither assigns a mode or triggers
escalation. Only an explicit authorized project policy may map these axes.
Preserve `MODE-UNBOUND` and the explicit `reasoningEffort` mapping rule.
`hook_observed` means only hook execution and `agent_declared` is assertion, not
verification. Hook order is not authority; portable instructions are not
enforcement, and Holytail never mutates third-party plugin state.
